...
class PolicyViewer:
    ...
    def _on_accept(self):
        ...
        # Write LOCAL permanent flag — prevents kiosk from running again on this PC
        try:
            local_flag_dir  = os.path.join(os.environ.get("LOCALAPPDATA","C:\\Temp"), "CBCPolicy")
            local_flag_file = os.path.join(local_flag_dir,
                f"accepted_{self.username}_{self.pc_name}_{self.policy_version}.flag")
            os.makedirs(local_flag_dir, exist_ok=True)
            with open(local_flag_file, "w") as f:
                f.write(f"username={self.username}\n")
                f.write(f"pc_name={self.pc_name}\n")
                f.write(f"ip={self.ip}\n")
                f.write(f"accepted_at={end_time.isoformat()}\n")
                f.write(f"policy_version={self.policy_version}\n")
                f.write(f"read_duration_sec={duration_sec}\n")
            print(f"Local flag saved: {local_flag_file}")
        except Exception as ex:
            print(f"Local flag error: {ex}")
...
def main():
    ...
    # Check BOTH local flag file AND database before showing kiosk
    # This works even when running PolicyViewer.py directly (without PS1)

    # 1. Check local flag file (fast, offline)
    local_flag_dir  = os.path.join(os.environ.get("LOCALAPPDATA","C:/Temp"), "CBCPolicy")
    local_flag_file = os.path.join(local_flag_dir, f"accepted_{args.user}_{args.pc}_{args.policy_version}.flag")

    if os.path.exists(local_flag_file):
        print(f"[SKIP] {args.user} already accepted policy {args.policy_version} on {args.pc}. Kiosk will not run.")
        sys.exit(0)

    # 2. Check database (covers cases where flag file was deleted)
    if os.path.exists(args.db):
        try:
            import sqlite3 as _sq
            con = _sq.connect(args.db, timeout=5)
            row = con.execute(
                "SELECT id FROM policy_acceptance WHERE username=? AND pc_name=? AND policy_version=? AND accepted=1",
                (args.user, args.pc, args.policy_version)
            ).fetchone()
            con.close()
            if row:
                # Recreate local flag so next check is faster
                os.makedirs(local_flag_dir, exist_ok=True)
                open(local_flag_file, "w").write("accepted")
                print(f"[SKIP] {args.user} already accepted (found in DB). Kiosk will not run.")
                sys.exit(0)
        except Exception as e:
            print(f"[WARN] DB check failed: {e} — continuing to show kiosk.")
